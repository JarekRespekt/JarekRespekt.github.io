let tg = window.Telegram.WebApp;

tg.expand();

tg.MainButton.textColor = "#FFFFFF";
tg.MainButton.color = "#2cab37";

let item = "";

let btn1 = document.getElementById("btn1");
let btn2 = document.getElementById("btn2");
let btn3 = document.getElementById("btn3");
let btn4 = document.getElementById("btn4");
let btn5 = document.getElementById("btn5");
let btn6 = document.getElementById("btn6");

let count1 = document.getElementById("count1");
let plusBtn1 = document.getElementById("plus1");
let minusBtn1 = document.getElementById("minus1");

btn1.addEventListener("click", function(){

	btn1.style.display = "none";
    count1.style.display = "inline";
    plusBtn1.style.display = "inline";
    minusBtn1.style.display = "inline";
    count1.innerText = "1";

	if (tg.MainButton.isVisible) {
		tg.MainButton.hide();
	}
	else {
		tg.MainButton.setText("Przejdź do podsumowania");
		item = "1";
		tg.MainButton. show();
	}
});

plusBtn1.addEventListener("click", function() {
    let currentCount = parseInt(count1.innerText);
    count1.innerText = (currentCount + 1).toString();
});

minusBtn1.addEventListener("click", function() {
    let currentCount = parseInt(count1.innerText);
    currentCount -= 1;
    if (currentCount === 0) {
        btn1.style.display = "inline";
        count1.style.display = "none";
        plusBtn1.style.display = "none";
        minusBtn1.style.display = "none";
    } else {
        count1.innerText = currentCount.toString();
    }
});

btn2.addEventListener("click", function(){
	if (tg.MainButton.isVisible) {
		tg.MainButton.hide();
	}
	else {
		tg.MainButton.setText("q");
		item = "2";
		tg.MainButton. show();
	}
});

btn3.addEventListener("click", function(){
	if (tg.MainButton.isVisible) {
		tg.MainButton.hide();
	}
	else {
		tg.MainButton.setText("q");
		item = "3";
		tg.MainButton. show();
	}
});

btn4.addEventListener("click", function(){
	if (tg.MainButton.isVisible) {
		tg.MainButton.hide();
	}
	else {
		tg.MainButton.setText("q");
		item = "4";
		tg.MainButton. show();
	}
});

btn5.addEventListener("click", function(){
	if (tg.MainButton.isVisible) {
		tg.MainButton.hide();
	}
	else {
		tg.MainButton.setText("q");
		item = "5";
		tg.MainButton. show();
	}
});

btn6.addEventListener("click", function(){
	if (tg.MainButton.isVisible) {
		tg.MainButton.hide();
	}
	else {
		tg.MainButton.setText("q");
		item = "6";
		tg.MainButton. show();
	}
});


Telegram.WebApp.onEvent ("mainButtonClicked", function(){
	tg. sendData(item);
});

let usercard = document.getElementById("usercard");

let p = document.createElement("p");

p.innerText = `${tg.initDataUnsafe.first_name}
${tg.initDataUnsafe.last_name}`;
usercard.appendChild(p);